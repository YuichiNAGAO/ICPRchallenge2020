...
if __name__ == "__main__":
    ...
    for fileidx in range(df_len):
        ...
        filling_level = df.iat[fileidx, 5]

        fi_type = [0] * 4
        fi_level = [0] * 3

        t1_gt_f[int(folder_num)].append(filling_level)
        t2_gt_f[int(folder_num)].append(filling_type)
        ...
        if mfcc_length < save_size:
            ...
            fi_level[int(filling_level)] += 1
            for i in range(save_mfcc_num):
                pos= 0
                if start_time/ap.signal_length_t*mfcc_length<i*f_step+f_length*0.75 and end_time/ap.signal_length_t*mfcc_length>i*f_step+f_length*0.25:
                    pos = 1

                fi_type[int(pos*filling_type)]+= 1
        print('fi_type: ', fi_type, '\tfi_level: ', fi_level)
    ...
    np.save(os.path.join(root_pth, 'audio', 't1_gt'), np.array(t1_gt_f))
    np.save(os.path.join(root_pth, 'audio', 't2_gt'), np.array(t2_gt_f))
