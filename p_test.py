...
"""
$ python test.py --root D:\codes\dataset --train-type part --folder_num 3 --val 3
"""

def compute_WAFS(gt, est):
	gt = gt.astype(str)
	est = est.astype(str)
	return f1_score(gt, est, average='weighted')

def compute_precision_recall_F1(gt, est):
	gt = gt.astype(str)
	est = est.astype(str)
	return precision_recall_fscore_support(gt, est, average='weighted')

def compute_acc(gt, est):
    gt = gt.astype(str)
	est = est.astype(str)
...
if __name__ == "__main__":
    ...
    t2_answer_list = []
    t1_answer_list = []
    t2_gt = np.load(os.path.join(root_dir, 'audio', 't2_gt.npy'))
    t1_gt = np.load(os.path.join(root_dir, 'audio', 't1_gt.npy'))
    ...
    for folder_num in args.folder_num:
        ...
        for file in sorted(files):
            ...
            t12_end = time.time()
            time_sum += (t12_end - t12_start)
            t2_answer_list.append(final_pred_T2)
            t1_answer_list.append(pred_T1)

        print('time_average:{}sec'.format(time_sum / count))
        t2_ans = np.array(t2_answer_list)
        t1_ans = np.array(t1_answer_list)

        t2_acc = compute_acc(t2_gt, t2_ans)
        t1_acc = compute_acc(t1_gt, t1_ans)

        t2_pre, t2_recall, t2_f, _ = compute_precision_recall_F1(t2_gt, t2_ans)
        t1_pre, t1_recall, t1_f, _ = compute_precision_recall_F1(t1_gt, t1_ans)

        t2_wafs = compute_WAFS(t2_gt, t2_ans)
        t1_wafs 0 compute_WAFS(t1_gt, t1_ans)

        print('t2 --- acc:{}, precision:{}, recall:{}, f1:{}, wafs:{}'.format(t2_acc, t2_pre, t2_recall, t2_f, t2_wafs))
        print('t1 --- acc:{}, precision:{}, recall:{}, f1:{}, wafs:{}'.format(t1_acc, t1_pre, t1_recall, t1_f, t1_wafs))
